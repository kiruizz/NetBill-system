                    ...
                    <form action="{{ route('clients.update', $client) }}" method="POST">
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control @error('national_id') is-invalid @enderror" 
                                           id="national_id" name="national_id" value="{{ old('national_id', $client->national_id) }}">
                                    @error('national_id')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                    ...
                                    <select class="form-select @error('service_plan_id') is-invalid @enderror" 
                                        ...
                                        @foreach($servicePlans as $plan)
                                            <option value="{{ $plan->id }}" 
                                                {{ old('service_plan_id', $client->activeSubscription ? $client->activeSubscription->service_plan_id : '') == $plan->id ? 'selected' : '' }}>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="date" class="form-control @error('connection_date') is-invalid @enderror" 
                                           id="connection_date" name="connection_date" value="{{ old('connection_date', $client->connection_date ? $client->connection_date->format('Y-m-d') : '') }}">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('notes') is-invalid @enderror" 
                                      id="notes" name="notes" rows="3" placeholder="Additional notes about this client...">{{ old('notes', $client->notes) }}</textarea>
                            @error('notes')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
